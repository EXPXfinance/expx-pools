<template>
  <Page>
    ...
    <UiButton
      v-if="!isTokensUsedUp && tokens.length < 8"
      class="mb-4"
      @click="addToken"
    >
      {{ $t('addToken') }}
...
export default {
  ...
  created() {
    ...
    const token1 = getTokenBySymbol('CAKE').address;
    const token2 = getTokenBySymbol('BUSD').address;
    this.tokens = [token1, token2];
    // weights contain percentage values - denorms are calculated later
    Vue.set(this.weights, token1, '60');
    Vue.set(this.weights, token2, '40');
    this.loading = false;
  },
  computed: {
    pool() {
