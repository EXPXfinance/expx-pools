<template>
  <div class="border rounded-1 panel-background p-4">
    <div
      v-if="ePool.metadata.rights.canPauseSwapping"
    ...
    <div
      v-if="ePool.metadata.rights.canChangeSwapFee"
      class="border-bottom mb-4 pb-3"
    >
      <div class="float-right">
    ...
    <div
      v-if="ePool.metadata.rights.canChangeWeights"
      class="border-bottom mb-4 pb-3"
    >
    ...
    <div
      v-if="ePool.metadata.rights.canAddRemoveTokens"
      class="border-bottom mb-4 pb-3"
    >
      <div class="float-right">
        <UiButton
      ...
      <div class="overflow-hidden">
        <span
          v-for="(token, i) in ePool.metadata.tokens"
          :key="i"
    ...
    <div
      v-if="ePool.metadata.rights.canChangeCap"
      class="border-bottom mb-4 pb-3"
    >
      <div class="float-right">
      ...
      <div class="text-white">
        <div v-if="pool.espCap === MAX" v-text="$t('unlimited')" />
        <div v-else v-text="_num(pool.espCap)" />
      </div>
    </div>
    <div
      v-if="ePool.metadata.rights.canWhitelistLPs"
      class="border-bottom mb-4 pb-3"
    >
      <div class="float-right">
        <UiButton
  ...
  methods: {
    ...
    async handlePokeWeights() {
      const txResult = await this.pokeWeights({
        poolAddress: this.ePool.metadata.controller
      });
