    ...
    <UiTable class="mb-4">
      ...
      <UiTableTr v-for="(balance, i) in balances" :key="i">
        ...
        <div class="flex-auto text-left">
          <div v-if="balance.address !== 'bsc'" class="flex-auto">
            <UiButton
  ...
  computed: {
    balances() {
      const balances = Object.entries(this.web3.balances)
        .filter(
          ([address]) => address !== 'bsc' && this.web3.tokenMetadata[address]
        )
      ...
      const bnbBalance = formatUnits(this.web3.balances['bsc'] || 0, 18);
