...
const actions = {
  ...
  createPool: async (
    ...
    try {
      ...
      const underlyingParams = [
        'EActions',
        config.addresses.eActions,
        'create',
        [config.addresses.eFactory, tokens, balances, weights, swapFee, true],
        {}
      ];
      console.log(
        'Create shared pool',
  ...
  pokeWeights: async ({ commit, dispatch }, { poolAddress }) => {
    ...
    try {
      const params = ['CRP', poolAddress, 'pokeWeights', [], {}];
      await dispatch('processTransaction', {
        params,
        title: 'Poke weights'
      });
  ...
  unwrap: async ({ commit, dispatch }, amount) => {
    ...
    try {
      ...
      dispatch('notify', [
        'green',
        `You've successfully unwrapped ${amount} bnb`
      ]);
