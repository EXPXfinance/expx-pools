...
import { multicall } from '@/expx/utils';
...
const actions = {
  ...
  initTokenMetadata: async ({ commit }) => {
    const invalids = [] as Array<string>;
  ...
  loadTokenMetadata: async ({ commit }, tokens) => {
    ...
    try {
      ...
      const calls = tokens
        ...
        .reduce((a, b) => [...a, ...b]);
      const res = await multicall(provider, abi['BEP20'], calls);
  ...
  getPoolBalances: async (_state, { poolAddress, tokens }) => {
    ...
    const calls = [];
    const bep20 = new Interface(abi.BEP20);
    const tokensToFetch = tokens
      ? tokens
      : Object.keys(state.balances).filter(token => token !== 'bsc');
    tokensToFetch.forEach(token => {
      calls.push([
        // @ts-ignore
        token,
        // @ts-ignore
        bep20.encodeFunctionData('balanceOf', [poolAddress])
      ]);
    });
    ...
    try {
      ...
      response.forEach(value => {
        if (tokensToFetch && tokensToFetch[i]) {
          const balanceNumber = bep20.decodeFunctionResult('balanceOf', value);
  ...
  getBalances: async ({ commit }, tokens) => {
    ...
    tokensToFetch.forEach(token => {
      // @ts-ignore
      calls.push([token, bep20.encodeFunctionData('balanceOf', [address])]);
    });
    promises.push(multi.aggregate(calls));
    ...
    try {
      ...
      // @ts-ignore
      balances.bsc = bnbBalance.toString();
      let i = 0;
      response.forEach(value => {
        if (tokensToFetch && tokensToFetch[i]) {
          const balanceNumber = bep20.decodeFunctionResult('balanceOf', value);
  ...
  getAllowances: async ({ commit }, tokens) => {
    ...
    tokens.forEach(token => {
      calls.push([
        ...
        token,
        // @ts-ignore
        bep20.encodeFunctionData('allowance', [address, spender])
      ]);
    });
    promises.push(multi.aggregate(calls));
    ...
    try {
      ...
      response.forEach(value => {
        if (tokens && tokens[i]) {
          const tokenAllowanceNumber = bep20.decodeFunctionResult(
            'allowance',
            value
          );
