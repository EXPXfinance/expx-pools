  ...
  computed: {
    ...
    tokenToSpend() {
      if (this.isWeightIncrease) {
        ...
        // If it is a EPT, we know it is 18 decimals - and it might not be in the metadata
    ...
    validUpdate() {
      ...
      // The unlock logic additionally ensures we have approval to transfer (either the token, or the EPT)

      if (this.is_valid) {
        // If it is a weight increase, we are depositing tokens and minting EPTs
        // If it is a weight decrease, we are withdrawing tokens and burning EPTs

        // The tokenToSpend logic only cares about which token we're depositing; won't be set if we're burning EPT
        ...
        const eptAmount = this.isWeightDecrease
          ? this.amountToSpend
          : calcPoolOutGivenWeightIncrease(
              toWei(totalWeight),
              toWei(selectedToken.denormWeight * this.divisor),
        ...
        return this.$t('infoWeightUpdate', {
          ...
          eptAction: this.isWeightDecrease ? this.$t('burn') : this.$t('mint'),
          eptAmount: eptAmount,
          eptSymbol: this.pool.symbol
        });
      }
