...
export default {
  ...
  computed: {
    poolTokenBalance() {
      const eptAddress = this.ePool.getEptAddress();
      const balance = this.web3.balances[getAddress(eptAddress)];
      return normalizeBalance(balance || '0', 18);
    },
    totalShares() {
      return this.ePool.metadata.totalShares;
    },
    userLiquidity() {
    ...
    requiredApprovals() {
      return Object.fromEntries(
        this.ePool.metadata.tokensList
          .filter(
            token =>
              this.isMultiAsset ||
  ...
  methods: {
    ...
    async handleSubmit() {
      this.loading = true;
      const poolAddress = this.ePool.getEptAddress();
      if (this.isMultiAsset) {
        const params = {
          poolAddress,
    ...
    async canAddLiquidity() {
      ...
      if (
        this.ePool.metadata.crp &&
        this.ePool.metadata.rights.canWhitelistLPs
      ) {
        // Need to check if this address is on the LP whitelist
        return await canProvideLiquidity(
          this.ePool.metadata.controller,
